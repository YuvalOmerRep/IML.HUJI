...
def select_polynomial_degree(n_samples: int = 100, noise: float = 5):
    ...
    go.Figure().add_trace(
        go.Scatter(x=X, y=y_no_noise, mode="markers")).add_trace(
        go.Scatter(x=X_train.to_numpy().T[0], y=y_train, mode="markers")).add_trace(
        go.Scatter(x=X_test.to_numpy().T[0], y=y_test, mode="markers")).update_layout(title=f"data with {noise} noise"
                                                                                      ).write_image(f"data{noise}.png")
    ...
    print(f"Best value of k is {best_eset.k} "
...
def select_regularization_parameter(n_samples: int = 50, n_evaluations: int = 500):
    ...
    alpha_x = np.arange(1, n_evaluations, 1, int) / 1000
    ...
    fig.add_trace(go.Scatter(x=alpha_x,
                             y=val_errors_ridge)).add_trace(go.Scatter(x=alpha_x, y=train_errors_ridge))

    fig.write_image("ridge_alpha_error.png")

    fig1 = go.Figure()

    fig1.add_trace(go.Scatter(x=alpha_x, y=val_errors_lasso)).add_trace(
        go.Scatter(x=alpha_x, y=train_errors_lasso))

    fig1.write_image("lasso_alpha_error.png")
